...
class MiniImageNetDataLoader(object):
    def __init__(self, shot_num, way_num, episode_test_sample_num, device, shuffle_images = False):
    ...
    def get_batch(self, phase='train', idx=0):
        ...
        this_inputa = torch.tensor(this_inputa).to(self.device)
        this_labela = torch.tensor(this_labela).to(self.device)
        this_inputb = torch.tensor(this_inputb).to(self.device)
        this_labelb = torch.tensor(this_labelb).to(self.device)
